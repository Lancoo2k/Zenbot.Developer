...
void Twinblades::MainExecuteCode() {

}
void Twinblades::FreeEvent() {

}
void Twinblades::LibraryInit() {

}
void Twinblades::ExecuteCode(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy) {
...
bool Twinblades::IsInAttackRange(float Distance, float ExtraRange) {
	return Distance <= ((SDK->KEYBOARD.Get_IsActiveLeft() ? LeftAARange : RightAARange) + ExtraRange);
}
int Twinblades::GetNextChargeType(LC_PLAYER_INFO* Enemy, bool EnemyIsDontAttack) {
	return 2;
}
bool Twinblades::IsFastStatck(LC_PLAYER_INFO* Player) {
	std::wstring Name;
...
bool Twinblades::Get_IsCharge(LC_PLAYER_INFO* Player) {
...
bool Twinblades::AttackShock(LC_PLAYER_INFO* LocalPlayer)
{
...
bool Twinblades::ExecuteShock(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy, Control& NextInput) {
...
bool Twinblades::Cab(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy, Control& NextInput) {
	if (SDK->KEYBOARD.Get_IsExecuteCombo()) {
		NextInput = Control::DirectInput;
		return false;
	}
	SDK->KEYBOARD.Set_IsExecuteCombo(true);
	if ((SDK->UTILITY.CheckInRange(Enemy->ActorModel, CARange) && SDK->KEYBOARD.BasicLogic.ExecuteCA())) {
		bool Logic1 = SDK->KEYBOARD.BasicLogic.StartWaitComboDelay(LocalPlayer, Enemy, 0.48f);
		bool Logic2 = SDK->GAMEFUNC.IsHiting(Enemy->ActorModel);
		if (!Logic1 || !Logic2) {
			if (!Logic2) {
				if ((SDK->GAMEFUNC.GetGlobalTime() - Enemy->BasicActiontime.LastFlashTime < 0.5f) ||
					(SDK->GAMEFUNC.GetGlobalTime() - Enemy->BasicActiontime.LastFlashstepTime < 0.5f)) {
					//对面闪了 就原地捏蓝防止再被反抓
					SDK->KEYBOARD.StopCombo();
					NextInput = Control::DirectInput;
					return false;
...
bool Twinblades::AllUse_AfterCombo(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy, Control& NextInput,bool disable) {
...
bool Twinblades::Right_ComboRun(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy, Control& NextInput, bool IsAAAfter) {
	if (SDK->KEYBOARD.Get_IsExecuteCombo()) {
		NextInput = Control::DirectInput;
		return false;
	}
...
bool Twinblades::Left_ComboRun(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy, Control& NextInput, bool IsAAAfter) {
	if (SDK->KEYBOARD.Get_IsExecuteCombo()) {
		NextInput = Control::DirectInput;
		return false;
	}
...
void Twinblades::Combo_Solo(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy) {
	...
	if (IsCharge) {
		...
		bool hasRiYue = SDK->GAMEFUNC.HasSoulItem(LocalPlayer->ActorModel, SoulItems::Twinblades_QianKunRiYueZhan);
		bool QianKunMode = hasRiYue && ChargeLevel == 2;
		...
		if (!EnemyIsDontAttack) {
			...
			if (Distance <= (A3Mode ? 5.5f : ChargeRange) && CanCastCharge && LastBladeParryTime > 1.2f) {
				...
				if (ShuiPaoMode ? ChargeLevel == 2 : CanNext) {
					...
					float AllowTime = SDK->GAMEFUNC.GetGlobalTime() + (QianKunMode ? 1.17f : (A3Mode ? 0.66f : (ChargeLevel == 1 ? 0.88f : 1.20f)));
					...
					bool iscast = false;
					if (!A3Mode && (SDK->ANIMHASH.IsHurtFlaying(Enemy->ActorModel) || SDK->ANIMHASH.IsHurtFlayingToLand(Enemy->ActorModel))) {
						iscast = true;
						//追加惊雷
						SDK->KEYBOARD.SendMouseEventBoard(MOUSEEVENTF_RIGHTDOWN);
						SDK->KEYBOARD.Set_IsActiveRight(true);
						...
						while (AllowTime > SDK->GAMEFUNC.GetGlobalTime()) {
							if (SDK->GAMEFUNC.GetActionName(LocalPlayer->ActorModel, Name)) {
								if (Name.find(skCrypt(L"twinblades_attack_heavy_04")) != std::wstring::npos) { //太刀右键_惊雷十劫(太刀蓄力之后的右键追击魂玉)
									SDK->KEYBOARD.SendMouseEventBoard(MOUSEEVENTF_RIGHTUP);
									SDK->KEYBOARD.Set_IsActiveRight(false);
									success = true;
									break;
						...
						if (success) {
							bool next = true;
							float AllowTime = SDK->GAMEFUNC.GetGlobalTime() + 0.75f;
							while (AllowTime > SDK->GAMEFUNC.GetGlobalTime()) {
								if (LocalPlayer->BasicPlayerInfo.IsShocked || LocalPlayer->BasicPlayerInfo.IsBounce) {
									//自己被振刀就跳出循环
									next = false;
							...
							if (next) {
								...
								Cab(LocalPlayer, Enemy, LastControl);
					...
					if (!iscast && SDK->GAMEFUNC.IsHiting(Enemy->ActorModel) && SDK->KEYBOARD.ActiveCombo(ActiveMode::All) && LocalPlayer->BasicPlayerInfo.CanExecuteFlash && !SDK->ANIMHASH.IsHurtFlayingToLand(Enemy->ActorModel) && !A3Mode && !Enemy->BasicPlayerInfo.IsBladeParry) {
						char InputKey = NULL;
						bool CanExecuteDoubleWeapon = false;
						auto WP = SDK->GAMEFUNC.GetDoubleWeaponID(&InputKey);
						auto RealWeaponID = SDK->UTILITY.RealWeaponToWeaponID(WP);
			...
			if (DontWhiteAttackTime < SDK->GAMEFUNC.GetGlobalTime() && !Enemy->BasicPlayerInfo.YinZiPing_F2 && !Enemy->BasicPlayerInfo.JiYingYing_Ghost) {
				...
				if (CanToAttack) {
					...
					bool Execute3 = AttackBlade(Enemy, true);
					...
					if (IsInAttackRange(Distance)) {
						...
						if (SDK->KEYBOARD.Get_IsActiveRight() && SDK->KEYBOARD.IsPressKey(2)) {
							...
							if ((Execute1 || Execute3) && Disable2A < SDK->GAMEFUNC.GetGlobalTime()) {
								if (StackValue == 2) {
									//处于一段攻击时
									SDK->KEYBOARD.SendMouseEventBoard(MOUSEEVENTF_RIGHTUP);
									SDK->KEYBOARD.Set_IsActiveRight(false);
...
void Twinblades::RunableLogic(LC_PLAYER_INFO* LocalPlayer, LC_PLAYER_INFO* Enemy) {
	//拼刀事件
	...
	if (IsCharge) {
		int ChargeLevel = SDK->GAMEFUNC.GetChargeLevel(LocalPlayer->ActorModel);
		bool hasRiYue = SDK->GAMEFUNC.HasSoulItem(LocalPlayer->ActorModel, SoulItems::Twinblades_QianKunRiYueZhan);
		bool soulMode = SDK->KEYBOARD.ActiveCombo(ActiveMode::SoulMode) && hasRiYue;
		bool QianKunMode = hasRiYue && ChargeLevel == 2;
		if (SDK->GAMEFUNC.PlayerIsCharge(LocalPlayer->ActorModel)) {
			if (Distance <= (QianKunMode ? 7.f : ChargeRange)) {
				if (CheckCanCastCharge(NearPosition, 5.f)) {
					if (!soulMode || QianKunMode) {
						if (SDK->KEYBOARD.IsPressKey(2) && SDK->KEYBOARD.Get_IsActiveRight()) {
							SDK->KEYBOARD.SendMouseEventBoard(MOUSEEVENTF_RIGHTUP);
							SDK->KEYBOARD.Set_IsActiveRight(false);
						}
						bool againexecute = true;
						float AllowTime = SDK->GAMEFUNC.GetGlobalTime() + (QianKunMode ? 1.17f : (ChargeLevel == 1 ? 0.81f : 1.13f));
						while (AllowTime > SDK->GAMEFUNC.GetGlobalTime()) {
							if (LocalPlayer->BasicPlayerInfo.IsShocked || LocalPlayer->BasicPlayerInfo.IsBounce) {
								//自己被振刀就跳出循环
								againexecute = false;
